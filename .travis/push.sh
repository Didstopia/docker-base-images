...
if [ "$UPDATE_UBUNTU_14_04" == "1" ]; then
    ...
    echo ""
    eval $(./docker-make.sh -f --detailed .docker-make.ubuntu-14-04.yml) >/dev/null 2>&1
    #echo -n "done!"
else
...
if [ "$UPDATE_UBUNTU_16_04" == "1" ]; then
    #echo -n "    > Building/pushing.. "
    echo ""
    eval $(./docker-make.sh -f --detailed .docker-make.ubuntu-16-04.yml) >/dev/null 2>&1
    #echo -n "done!"
else
    echo -n "    > No update necessary, skipping.."
fi
...
if [ "$UPDATE_ALPINE_3_5" == "1" ]; then
    #echo -n "    > Building/pushing.. "
    echo ""
    eval $(./docker-make.sh -f --detailed .docker-make.alpine-3-5.yml) >/dev/null 2>&1
    #echo -n "done!"
else
    echo -n "    > No update necessary, skipping.."
fi
